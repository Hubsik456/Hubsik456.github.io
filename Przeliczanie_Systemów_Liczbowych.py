"""
    ...
    FIXME:
        Przy klikaniu kilka(naście) razy w ustawieniach żeby włączyć/wyłączyć historię, żeby nie dopisywało --- po kilka razy
"""
...
def Przeliczanie_Systemow_Liczbowych():
    ...
    def Walidacja():
        ...
        if Docelowe_Systemy == "": #? W Ogóle Podane Docelowe Systemy Liczbowe Lub Jeśli Podano Tekst
            Poprawne_Dane = False
        ...
        if Input == "": #? Czy W ogóle Podana Liczba
            Poprawne_Dane = False
        ...
        if Poprawny_System: #? Czy Input Zawiera Złe Cyfry (np. Czy liczba 123 (2) zawiera tylko "0" i "1")
            ...
            for x in range(len(Input)):
                if not Input[x] in Liczby[0:System]:
    ...
    def Przeliczanie_Na_Sys10(Input, System):
        ...
        if System == 10:
            ...
            for x in range(len(str(Input))):
                #print(f"{Liczby.index(str(Input)[x])} * {System ** (len(str(Input)) - x-1)}")
                Wynik += Liczby.index((str(Input)[x])) * (System ** (len(str(Input)) - x - 1))  #Pierwsze x to liczby od początku (lewej) drugie x liczby od końca (prawej)
    ...
    def Przeliczanie_Na_Inne_Systemy(Liczba, System):
        ...
        #Poprzedni = Liczba
        #Reszta = 0
        #Czy_Da_Sie_Dalej_Dzielic_Z_Reszta = True

        """
        #! v1
        if System in Ustawienia["Specjalne Systemy Liczbowe"]: #FIXME:
            return ["--- WIP ---", System]
        else:
            while Czy_Da_Sie_Dalej_Dzielic_Z_Reszta:
                if Poprzedni == 0:  #Koniec
                    Wynik = Wynik[::-1]  #Odwraca
                    Czy_Da_Sie_Dalej_Dzielic_Z_Reszta = False
                else:
                    Reszta = Liczby[Liczby.index(Liczby[Poprzedni % System])]

                    #print(f"{Poprzedni} / {System} = {floor(Poprzedni / System)} r {Reszta}")
                    Poprzedni = floor(Poprzedni / System)
                    Wynik += str(Reszta)
...
if __name__ == "__main__":
    ...
    Menu_Settings.add_checkbutton(label="WIP | Obliczenia", onvalue=1, offvalue=0, variable=M_Obliczenia, command=PARTIAL(Aktualizacja_Ustawien, "Wyświetlaj Obliczenia"))
    ...
    Menu_Linki.add_command(label="exploringbinary.com (Decimal -> Float Point)", command=PARTIAL(WEBBROWSER.open, "https://www.exploringbinary.com/floating-point-converter/"))
        #? End
